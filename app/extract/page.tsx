...
export default function ExtractPage() {
  ...
  const [gameEmojis, setGameEmojis] = useState<
    { id: number; emoji: string; x: number; caught: boolean }[]
  >([]);
    ...
    <div className="min-h-screen bg-[#f9f9ff] pb-24">
      ...
      {loading && (
        <div className="fixed inset-0 z-40 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center px-8">
          ...
          <p className="text-2xl font-bold text-zinc-900 mb-2 text-center">
            Extracting your actions...
          </p>
      ...
      <main className="max-w-2xl mx-auto px-4 pt-5">
        <p className="text-zinc-500 text-sm mb-5">
          Use any format. AI handles the rest.
        </p>
        ...
        <div className="flex items-center gap-1 overflow-x-auto border-b border-zinc-200 mb-6 no-scrollbar">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-1.5 py-3 px-3 border-b-2 whitespace-nowrap transition-all text-sm font-medium ${
                activeTab === tab.id
          ...
          <div>
            <div className="relative bg-white border border-zinc-200 rounded-2xl p-4 mb-4 shadow-sm">
              ...
              <input
                ...
                className="w-full border border-zinc-200 rounded-xl px-4 py-3 text-zinc-800 placeholder-zinc-300 text-sm focus:outline-none focus:border-zinc-400 transition"
              />
              <p className="text-xs text-zinc-400 mt-2">
                Works with articles, changelogs, job posts, competitor pages
              </p>
            </div>
            <button
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (f) handleFileUpload(f, "/api/parse-image");
              }}
            />
            <input
              ref={whatsappRef}
            ...
            <input
              ...
              onChange={(e) => {
                const f = e.target.files?.[0];
                if (f) handleFileUpload(f, "/api/parse-document");
              }}
            />
            <input
              ref={voiceRef}
        ...
        {!showResults && (
          <div className="mt-8">
            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.15em] mb-4">
              Quick Templates
            </p>
            <div className="grid grid-cols-2 gap-3">
              {QUICK_TEMPLATES.map((t) => (
                ...
                >
                  <span className="material-symbols-outlined text-zinc-400 group-hover:text-zinc-700 text-[22px] mb-2 block transition-colors">
                    {t.icon}
                  </span>
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2 flex-1">
          <span
            className={`material-symbols-outlined text-[18px] mt-0.5 shrink-0 ${tc.color}`}
          >
            {tc.icon}
          </span>
        ...
        <span
          className={`text-[11px] px-2 py-0.5 rounded-full font-semibold shrink-0 ${priorityColors[action.priority]}`}
        >
          {action.priority}
        </span>
      </div>
