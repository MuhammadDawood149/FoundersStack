...
function getCardStyle(
  action: Action,
  deadlineState: ReturnType<typeof getDeadlineState>,
) {
  if (action.status === "done") return "bg-zinc-50 border-zinc-200 opacity-70";
  if (action.status === "snoozed")
    return "bg-zinc-50 border-zinc-200 opacity-60";
  if (deadlineState === "overdue") return "bg-zinc-100 border-zinc-200";
  if (deadlineState === "today")
    return "bg-[#FEE2E2] border-[#FCA5A5] border-l-4 border-l-red-600";
  if (deadlineState === "soon")
    return "bg-[#FFF7ED] border-[#FED7AA] border-l-4 border-l-orange-500";
...
export default function ActionsPage() {
  ...
  const filtered = actions.filter((a) => {
    ...
    if (filter === "follow-ups")
      return a.type === "follow-up" && a.status === "todo";
    if (filter === "reminders")
      return a.type === "reminder" && a.status === "todo";
    ...
    <div className="min-h-screen bg-[#f9f9ff] pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-5">
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
          {FILTERS.map((f) => (
            ...
            >
              ...
              {counts[f.id] > 0 && (
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${filter === f.id ? "bg-white/20 text-white" : "bg-zinc-100 text-zinc-500"}`}
                >
                  {counts[f.id]}
                </span>
              )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-zinc-400">
            <span className="material-symbols-outlined text-5xl mb-4 text-zinc-300">
              inbox
            </span>
            <p className="font-semibold text-zinc-500">No actions here</p>
            <p className="text-sm text-zinc-400 mt-1">
              {filter === "all"
                ? "Extract some thoughts to get started"
                : `No ${filter} actions right now`}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((action) => {
              ...
              return (
                <div
                  key={action.id}
                  className={`border rounded-2xl p-4 shadow-sm transition-all ${cardStyle}`}
                >
                  {/* Top row */}
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-lg font-semibold ${tc.bg}`}
                      >
                        {action.type}
                      </span>
                      ...
                      {deadlineState === "soon" && (
                        <span className="text-xs px-2 py-0.5 rounded-lg font-semibold bg-orange-100 text-orange-700">
                          Soon
                        </span>
                      )}
                    </div>
                    <span
                      className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${priorityColors[action.priority]}`}
                    >
                      {action.priority}
                    </span>
                  </div>
                  ...
                  {/* Title */}
                  <h3
                    className={`font-semibold text-base leading-snug mb-2 ${isDone || deadlineState === "overdue" ? "line-through text-zinc-400" : "text-zinc-900"}`}
                  >
                    {action.title}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-3 mb-4">
                    ...
                    {action.due_date && (
                      <span
                        className={`text-xs flex items-center gap-1 ${deadlineState === "overdue" ? "text-zinc-400" : deadlineState === "today" ? "text-red-600 font-medium" : deadlineState === "soon" ? "text-orange-600" : "text-zinc-500"}`}
                      >
                        <span className="material-symbols-outlined text-[14px]">
                          schedule
                        </span>
